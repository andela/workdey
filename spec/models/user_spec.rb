...
RSpec.describe User, type: :model do
  ...
  it { is_expected.to have_many(:taskees).class_name("TaskManagement").with_foreign_key(:taskee_id) }
  it { is_expected.to have_many(:taskers).class_name("TaskManagement").with_foreign_key(:tasker_id) }
  ...
  describe ".validate_email" do
    ...
    it "must be unique" do
      user = create(:user)
      user2 = User.new(firstname: "Ikem", lastname: "Okonkwo", email: "mayowa.pitan@andela.com", password: "ruby_tabernacle")
      expect(user2.save).to eql false
    end
    it "must not contain odd characters" do
      expect(build(:user, email: "mayowa.pitan()@.andela.com").save).to eql false
    end
  end
  ...
  pending ".first_or_create_from_oauth" do
   ...
   describe ".get_user_address" do
    it "should return the user's address" do
      ...
      expect(User.get_user_address("mayowa.pitan@andela.com").first).to eql ["Lagos", "55, Moleye Str"]
    end
   end
   describe ".get_taskees_by_task_name" do
    it "can return users by their task name" do
      user1 = create(:user)
      user2 = create(:user_with_tasks, email: "ikem.okonkwo@andela.com")
      user3 = create(:user_with_tasks, email: "bukola.makinwa@andela.com")
